...
let getVowelsCount = function (s) {
    ...
    for (let i of s) {
        ...
        else if(isEnglishAlphabet(i)){
            consonant++;
        }
        
    }
    return { vowelsCount, consonant };
};
var vowelConsonantScore = function (s) {
    let sLen = s.length;
    let { vowelsCount, consonant } = getVowelsCount(s);
    if (consonant > 0) {
        return Math.floor(vowelsCount / consonant);
    }
    return 0;
};
